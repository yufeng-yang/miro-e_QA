...
class DDAMNet(nn.Module):
    '''
    这是被我魔改过的版本，原始版本在bin/DDAMFN_NETWORK.py

    这里的改动包括：预训练模型的路径。2. 返回值的改动，增加了return_dealt_feature参数，用于返回处理后的特征。
    '''
    def __init__(self, num_class=7,num_head=2, pretrained=True):
        ...
        # 这个路径必须要改        
        if pretrained:
            net = torch.load(os.path.join('/home/yufeng/mdk/bin/networks/pretained/MFN_msceleb.pth')
)      
