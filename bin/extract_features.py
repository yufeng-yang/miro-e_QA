...
# 卷积核的定义类，定义的卷积核是1*1的，并且得益于stride=(1, 1)，即每次只平移一格单位（准确来说一个像素），这就导致输出大小与原来相同（长宽上来说），也就是没有降采样
...
# 这就是那个最后能得到叫嵌入向量或 embedding的代码，也就是所谓的deep embedding
# 将输入图像转换为一个固定大小的特征向量，这里的embedding_size被设置为256
