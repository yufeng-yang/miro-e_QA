...
# 上面是用作识别以带音频的视频作为输入格式的抓取特征，但在我的应用场景下，声音和视频的来源是分开的，所以需要分开抓取特征
def extract_solo_audio_feature(audio_model, audio_path):
    """
    提取音频特征。
    ...
    Returns:
        torch.Tensor: 音频特征，形状为 [1, 768]。作为feature
        以及音频的标签和得分，作为full
    """
    audio_output = extract_audio_feature(audio_model, audio_path)
    # 提取标签和得分
    audio_label_and_score = {
        "labels": audio_output[0]['labels'],
        "scores": audio_output[0]['scores']
    }
    audio_feature = torch.tensor(audio_output[0]['feats'], dtype=torch.float32)
    audio_feature = audio_feature / torch.norm(audio_feature, p=2)
    audio_feature = audio_feature.unsqueeze(0)  # Shape: [1, 768]
    return audio_feature, audio_label_and_score

def extract_solo_video_feature(mediapipe_model, emotion_model, video_path, frame_interval=2):
    """
    提取视频特征。
...
# # ====================================================================================================
# # 下面是测试代码（注意audio的修改会导致目前代码处于失效状态）

# # 测试一下分开提取后再拼接与原本一次性提取的结果是否一致
# audio_model = initionalize_audio_model()
# mediapipe_model = initialize_face_detector()
# emotion_model = load_emotion_model()
# video_path = "/home/yufeng/mdk/bin/01-01-03-02-01-01-01.mp4"
...
# print(fused_feature1.shape)
# # # 手动将音频从视频中提取出来
# audio_path = extract_audio_from_video(video_path)

# # 手动抓取音频特征和视频特征
# audio_feature = extract_solo_audio_feature(audio_model, audio_path)
# video_feature = extract_solo_video_feature(mediapipe_model, emotion_model, video_path, frame_interval=2)

# # 融合特征
# fused_feature2 = torch.cat([video_feature, audio_feature], dim=-1)  # Shape: [1, 1280]
# print(torch.allclose(fused_feature1, fused_feature2))  # True
# # ====================================================================================================

# audio_model = initionalize_audio_model()
# audio_path = "/home/yufeng/mdk/captured_audio.wav"
