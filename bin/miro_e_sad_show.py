...
class SadMoodFeedback:
    def __init__(self, interface, robot_state):
        self.interface = interface
        ...
        # 用于记录当前机器人的状态
        self.inside_sad_emotion = robot_state
    ...
    # 下面是尾巴的控制代码
    def tail_function(self, duration=8, frequency=0.5, amplitude=1.0, default_wag=0.5, default_droop=0.5):
        """
   ...
   ##################################################################################################################
    ...
    def ear_flap_function(self, duration=3, frequency=0.5, amplitude=0.5):
        """
        控制耳朵上下摆动，模拟情绪反应。
        - duration: 持续时间（秒）。
        - frequency: 摆动频率（每秒）。
    ...
    def eye_blink_function(self, duration=5, blink_interval=1.0):
        ...
        while time.time() - start_time < duration:
            # 闭眼
            self.interface.set_cos(miro.constants.JOINT_EYE_L, 0.8)
            self.interface.set_cos(miro.constants.JOINT_EYE_R, 0.8)
            time.sleep(0.1)  # 闭眼 0.1 秒

            # 睁眼
            self.interface.set_cos(miro.constants.JOINT_EYE_L, 0.5)
            self.interface.set_cos(miro.constants.JOINT_EYE_R, 0.5)
    ...
    def head_high(self, duration=5):
        self.interface.set_kin(miro.constants.JOINT_PITCH, 0.1)
        time.sleep(duration)
    ...
    def happy_audio(self, sound_file):
        ...
        playback.play()
    #############################################################################################################################
    # 为了增强交互效果，我决定检测用户是否用手摸机器人，为是后再摇动尾巴和耳朵，为否就停止摇动
    ...
    def touch_and_feedback(self):
        while self.inside_sad_emotion == 'Sad':
...
def execute_all_sad_functions(controller, duration=5):
    """
    多线程同时执行所有功能。
    ...
    # tail_thread = threading.Thread(target=controller.tail_function, args=(duration,))
    # ear_thread = threading.Thread(target=controller.ear_flap_function, args=(duration,))
    eye_thread = threading.Thread(target=controller.eye_blink_function, args=(duration,))
    head_thread = threading.Thread(target=controller.head_high, args=(duration,))
    # sound_thread = threading.Thread(target=controller.happy_audio, args=("/home/yufeng/mdk/sad_dog.wav",))
    touch_thread = threading.Thread(target=controller.touch_and_feedback)
...
if __name__ == "__main__":
    ...
    # 创建控制器对象
    controller = SadMoodFeedback(interface,'Sad')
