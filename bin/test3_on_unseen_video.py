...
video_path = "C:\\Users\\28402\\Pictures\\Camera Roll\\angry_test.mp4"
...
# video_path = r'C:\CodeSpace\training_source\video_sources\Video_Speech_Actor_03\Actor_03\02-01-04-01-01-01-03.mp4'
# video_path = r'C:\CodeSpace\integrate_video_audio_features\01-01-05-02-01-02-02.mp4'
...
#         self.input_size = input_size  # 动态输入特征维度
...
#     def forward(self, x):
#         assert x.shape[1] == self.input_size, f"Input feature size mismatch: {x.shape[1]} != {self.input_size}"
#         return self.model(x)
...
class EmotionClassifier(nn.Module):
    def __init__(self, input_size, num_classes):
        super(EmotionClassifier, self).__init__()
        self.model = nn.Sequential(
            nn.Linear(input_size, 512),  # 第一隐藏层
            nn.ReLU(),
            nn.BatchNorm1d(512),         # 添加 BatchNorm
            nn.Linear(512, 256),         # 第二隐藏层
            nn.ReLU(),
            nn.BatchNorm1d(256),
            nn.Linear(256, 128),         # 第三隐藏层
            nn.ReLU(),
            nn.Dropout(0.05),            # 降低 Dropout
...
# 加载训练好的模型权重
model.load_state_dict(torch.load(r'C:\CodeSpace\integrate_video_audio_features\emotion_classifier_all_best.pth'))
model.eval()  # 切换到评估模式
